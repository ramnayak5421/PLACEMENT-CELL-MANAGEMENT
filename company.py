...
smtp_port = 587 
smtp_username = "pythonproject.itworkshop"
smtp_password = "abnr fvwp dvkw vyvs"



def send_email(company_name, company_email, student_email):
    subject = "Job Opportunity at {}".format(company_name)
    body = "Dear Student,\n\n" \
           "A job opportunity is available at {}.\n" \
           "Please contact the company at {} for more information.\n\n" \
           "Best regards,\nYour Placement Team".format(company_name, company_email)
    ...
    try:
        ...
        # Send the email
        server.sendmail(smtp_username, student_email, f"Subject: {subject}\n\n{body}")
...
def DisplayData():
    mydb = mysql.connector.connect(host='localhost', user='root', password='csprogrammer', database='placement')
    cur = mydb.cursor()
...
def submit():
    ...
    cur.execute("select email from student_table")
    student_emails = [row[0] for row in cur.fetchall()]

    for student_email in student_emails:
        send_email(company_name_var.get(), company_email_var.get(), student_email)
...
def on_entry_click(event):
    if search_text.get() == "Enter Company ID":
        search_text.delete(0, "end")
        search_text.config(fg='black')

def on_focusout(event):
    if search_text.get() == "":
        search_text.insert(0, "Enter Company ID")
        search_text.config(fg='grey')
...
search_var = StringVar()
search_text = Entry(search_frame, width=50, textvariable=search_var, fg='grey')
search_text.insert(0, "Enter Company ID")  
search_text.place(x=0, y=20)

search_text.bind("<FocusIn>", on_entry_click)   
search_text.bind("<FocusOut>", on_focusout)
