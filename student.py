...
branchE = ttk.Combobox(f2, textvariable=bran_var, width=25)
...
def on_entry_click(event):
    if search_text.get() == "Enter Student Registration ID":
        search_text.delete(0, "end")  # Delete the default text
        search_text.config(fg='black')  # Change text color to black

def on_focusout(event):
    if search_text.get() == "":
        search_text.insert(0, "Enter Student Registration ID")  # Insert default text
        search_text.config(fg='grey')  # Change text color back to grey
...
search_text.insert(0, "Enter Student Registration ID")  # Set default text
search_text.place(x=0, y=20)

search_text.bind("<FocusIn>", on_entry_click)     # Bind click event
